...
import RemoteTarget, {ContainerTransport, RemoteContainerTransport} from '#src/main.ts'
...
integration('real Docker container transport', () => {
  ...
  let remoteTransport: RemoteContainerTransport
  let target: RemoteTarget
  beforeAll(async () => {
    await docker(['info', '--format', '{{.ServerVersion}}'])
    endpoint = process.env.DOCKER_HOST && !process.env.DOCKER_CONTEXT ? process.env.DOCKER_HOST : await docker(['context', 'inspect', '--format', '{{.Endpoints.docker.Host}}'])
    ...
    remoteTransport = new RemoteContainerTransport(containerId, endpoint, {user: '1000:1000'})
  ...
  test('inherited Docker configuration and explicit endpoints reach the same container ID', async () => {
    for (const selected of [transport, remoteTransport, new ContainerTransport(containerName)]) {
  ...
  test('direct exec preserves complex argv without a shell', async () => {
    ...
    const result = await remoteTransport.runShellNeutralCommand(['bun', '--eval', 'console.log(JSON.stringify(Bun.argv.slice(1)))', ...values], {timeoutMs: 10_000})
    expect(result.exitCode).toBe(0)
  ...
  test('stdin round trips NUL, Unicode and empty input without a TTY', async () => {
    for (const stdin of ['', 'NUL\0\u2192\u{1F9D9}\n'.repeat(20_000)]) {
      const result = await remoteTransport.runShellNeutralCommand(['cat'], {
        stdin,
  ...
  test('Mage-style direct runtime invocation does not interpret application protocol output', async () => {
    const result = await remoteTransport.runShellNeutralCommand([target.getRuntime().file, '-'], {
  ...
  test('shell calls, nonzero exits and output limits use the existing transport contracts', async () => {
    const shell = await remoteTransport.runShellCommand('printf hello; printf warning >&2; exit 7', {timeoutMs: 10_000})
    expect(shell.exitCode).toBe(7)
    expect(shell.stdout).toBe('hello')
    expect(shell.stderr).toBe('warning')
    const limited = await remoteTransport.runShellNeutralCommand(['bun', '--eval', 'process.stdout.write("123456789")'], {
      maxOutputBytes: 3,
      timeoutMs: 10_000,
    })
  ...
  test('timeouts and cancellation bound the CLI without claiming container-process termination', async () => {
    ...
    const timedOut = await remoteTransport.runShellNeutralCommand(command, {timeoutMs: 500})
    expect(timedOut.exitCode).toBe(124)
    expect(timedOut.failure).toBe('timeout')
    const controller = new AbortController
    const reason = new Error('cancel Docker exec')
    await expect(remoteTransport.runShellNeutralCommand(command, {
